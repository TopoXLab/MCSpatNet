...
in_dir = '../../datasets/CoNSeP/Train'
out_root_dir = '../../datasets/CoNSeP_train'
# Define cell classes
classes_max_indx = 3
...
Grouped cell Classes: 
	      ...
	      2 = healthy epithelial + dysplastic/malignant epithelial (red)
          3 = fibroblast +  muscle  + endothelial + other (green) 
'''
...
def gaussian_filter_density(img, points, point_class_map, out_filepath, start_y=0, start_x=0, end_y=-1, end_x=-1):
    ...
    for s in range(1, density_class.shape[-1]):
        io.imsave(out_filepath.replace('.npy', '_s' + str(s) + '_solid.png'),
...
if __name__ == "__main__":
    ...
    for svs_path in svs_list:
        ...
        for patch_img_path in patches_img_list:
            ...
            patch_label_arr_dots_grouped = np.zeros((img2.shape[0], img2.shape[1], n_grouped_class_channels),
                                                    dtype=np.uint8)
            for class_id, map_class_lst in class_group_mapping_dict.items():
                ...
                img3[np.where(patch_label_arr > 0)] = color_set_grouping_dict[class_id]
            ...
            for dot_class in range(1, patch_label_arr_dots.shape[-1]):
                ...
                patch_label_arr = ndimage.convolve(patch_label_arr, np.ones((5, 5)), mode='constant', cval=0.0)
                img2[np.where(patch_label_arr > 0)] = color_set[dot_class]
            io.imsave(os.path.join(out_gt_dir, svs_name + '_' + im_filename[:-len('.png')] + '_img_with_dots.jpg'),
                      img2)
