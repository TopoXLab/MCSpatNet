...
class CellsDataset(Dataset):
    def __init__(self,img_root, gt_dmap_root, gt_dots_root, class_indx, gt_dmap_subclasses_root, gt_dots_subclasses_root, gt_kmap_root, split_filepath=None, phase='train', fixed_size=-1, max_side=-1, max_scale=-1):
        ...
        gt_dmap_subclasses_root: the root path of the pseudo ground-truth clustering subclasses dilated dot maps.
        gt_dots_subclasses_root: the root path of the pseudo ground-truth clustering subclasses dot maps.
        gt_kmap_root: the root path of the ground-truth cross k-function maps.
    ...
    def __getitem__(self,index):
        ...
        # Read ground truth cross k-function map
        ...
        # Read pseudo ground truth class sub clusters dot maps
        ...
        if self.max_scale>1: # to downsample image and density-map to match deep-model.
            ...
            gt_subclasses_dmap = np.pad(gt_subclasses_dmap, ((pad_y1,pad_y2),(pad_x1,pad_x2),(0,0)), 'constant', constant_values=(0,) )# padding constant differs by dataset based on bg color
            gt_subclasses_dots = np.pad(gt_subclasses_dots, ((pad_y1,pad_y2),(pad_x1,pad_x2),(0,0)), 'constant', constant_values=(0,) )# padding constant differs by dataset based on bg color
            gt_kmap = np.pad(gt_kmap, ((pad_y1,pad_y2),(pad_x1,pad_x2),(0,0)), 'constant', constant_values=(0,) )# padding constant differs by dataset based on bg color


        # Covert image and ground truth to pytorch format
        img=img.transpose((2,0,1)) # convert to order (channel,rows,cols)
        gt_kmap=gt_kmap.transpose((2,0,1)) # convert to order (channel,rows,cols)
        if(len(self.class_indx_list) > 1):
            ...
            gt_subclasses_dmap=gt_subclasses_dmap.transpose((2,0,1)) # convert to order (channel,rows,cols)
            gt_subclasses_dots=gt_subclasses_dots.transpose((2,0,1)) # convert to order (channel,rows,cols)
        else:
